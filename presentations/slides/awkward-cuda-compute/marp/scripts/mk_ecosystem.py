...
W=total+0.5; x0=(W-total)/2; cxp=W/2   # width-aware canvas so every box fits fully
fig,ax=plt.subplots(figsize=(W,4.6)); ax.set_xlim(0,W); ax.set_ylim(0,4.6); ax.axis("off")
# parent
ax.add_patch(FancyBboxPatch((cxp-1.5,3.7),3.0,0.7,boxstyle="round,pad=0.02,rounding_size=0.1",fc="#e8eef2",ec=INK,lw=2))
ax.text(cxp,4.05,"CUDA Python",ha="center",va="center",fontsize=16.8,fontweight="bold",color=INK)
for i,(name,blurb,fc,ec) in enumerate(libs):
    ...
    ax.add_patch(FancyArrowPatch((cxp,3.7),(x+w/2,y+h),arrowstyle="-|>",mutation_scale=12,lw=1.3,color=GREY))
# highlight this talk
